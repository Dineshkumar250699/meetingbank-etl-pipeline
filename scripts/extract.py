...
class MeetingBankExtractor:
    ...
    def fetch_dataset_from_huggingface(self, subset_size: int = None) -> List[Dict]:
        ...
        # Add this inside the loop to see what fields actually exist
        logger.info(f"Fetching meetings from HuggingFace...")
        ...
        try:
            ...
            meetings = []
            for item in dataset:
                logger.info(f"Keys found in HuggingFace: {item.keys()}")
                ...
                city = raw_city_part.replace('CityCouncil', '').replace('CC', '')
    ...
    def filter_by_cities(self, meetings: List[Dict]) -> List[Dict]:
        ...
        unique_cities_found = set(m.get('city') for m in meetings)
        logger.info(f"Actual cities found in raw data: {unique_cities_found}")
